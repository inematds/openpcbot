...
async function main() {
  ...
  console.log(`  ${c.bold}Welcome to OpenPCBot.${c.reset}`);
  console.log();
  ...
  // ── 2. What is OpenPCBot ────────────────────────────────────────────────
  section('What is OpenPCBot?');

  console.log(`  OpenPCBot bridges your Claude Code CLI to Telegram.`);
  console.log(`  You message your bot from your phone. OpenPCBot runs the`);
  ...
  info('OpenPCBot has several optional features. Tell us what you want.');
  ...
  if (wantWhatsApp) {
    ...
    console.log();
    info('OpenPCBot uses whatsapp-web.js to connect to your existing WhatsApp');
  ...
  info('OpenPCBot is one of several "Claw" projects. You might want to');
  ...
  info('OpenPCBot auto-loads every skill in ~/.claude/skills/.');
  ...
  if (wantVideo) {
    ...
    console.log();
    info('OpenPCBot\'s video analysis uses the gemini-api-dev skill from Google.');
  ...
  info('By default, OpenPCBot uses your existing claude login (Max plan).');
  ...
  const lines = [
    '# OpenPCBot — generated by setup wizard',
  ...
  info('OpenPCBot can run specialist agents alongside the main bot.');
  ...
  console.log(`  ${c.cyan}║${c.reset}${c.bold}           OpenPCBot is ready!             ${c.reset}${c.cyan}║${c.reset}`);
  console.log(`  ${c.cyan}╚════════════════════════════════════════════╝${c.reset}`);
  console.log();
...
function setupWindows() {
  ...
  info('  Install WSL2, clone OpenPCBot inside the WSL2 filesystem,');
